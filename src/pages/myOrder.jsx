            ...
            <div className="w-full overflow-x-auto">
                {loading ? (
                            ...
                            <tbody key={order.id}>
                                {order.order_items.map((product, idx) => (
                                    <tr key={order.id + '-' + idx} className="border-b border-[var(--bg-tertiary)]">
                                        ...
                                        <td className="px-2 py-3">
                                            <span className={`px-3 py-1 rounded-xl font-[poppins-medium] ${order?.status == 'shipped' && 'bg-[var(--color-primary)]/30 text-[var(--color-primary)]'} ${order.status == 'paid' ? 'bg-orange-400/30' : 'bg-[var(--bg-tertiary)] text-[var(--text-secondary)]'} font-medium  text-xs whitespace-nowrap`}>
                                                {order?.status}
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div className="flex flex-col justify-center py-16">
                        <span className="text-gray-500 mb-4">No orders found.</span>
                        <Button onClick={() => navigate('/products')} className="bg-[var(--color-primary)] text-white px-6 py-2 rounded-lg">
                            Continue Shopping
                        </Button>
                    </div>
                )}
